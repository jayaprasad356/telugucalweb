...
if (isset($_POST['btnUpdate'])) {
    ...
    if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
        ...
        $sql = "UPDATE mahabharatham SET `image`='$upload_image' WHERE `id`='$ID'";
        $db->sql($sql);
        ...
        if ($update_result == 1) {
            $error['update_mahabharatham'] = " <section class='content-header'><span class='label label-success'>Mahabharatham updated Successfully</span></section>";
        } else {
            $error['update_mahabharatham'] = " <span class='label label-danger'>Failed to update</span>";
        }
    }
}
...
<script>
    $('#edit_mahabharatham_form').validate({
        ignore: [],
        debug: false,
        rules: {
            title: "required",
