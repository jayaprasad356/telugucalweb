...
if (isset($_POST['btnEdit'])) {
		...
		if (!empty($name)) {
			if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
				...
				$target_path = 'upload/video-category/';
				...
				$upload_image = 'upload/video-category/' . $filename;
				$sql = "UPDATE image_category SET `image`='" . $upload_image . "' WHERE `id`=" . $ID;
				$db->sql($sql);
			}
			
             $sql_query = "UPDATE video_category SET name='$name' WHERE id =  $ID";
			 $db->sql($sql_query);
			...
			// check update result
			if ($update_result == 1) {
				$error['update_category'] = " <section class='content-header'><span class='label label-success'>Video Category updated Successfully</span></section>";
			} else {
...
$sql_query = "SELECT * FROM video_category WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "video-category.php";
	</script>
<?php } ?>
<section class="content-header">
	<h1>
		Edit Video Category<small><a href='video-category.php'><i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Video Category</a></small></h1>
