...
if (isset($_POST['btnAdd'])) {
    ...
    if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
        ...
        $sql = "INSERT INTO bhagawath_geetha (title, image) VALUES ('$title', '$upload_image')";
        $db->sql($sql);
    } else {
        // Image is not uploaded or empty, insert only the title
        $sql = "INSERT INTO bhagawath_geetha (title) VALUES ('$title')";
        $db->sql($sql);
    }
    $result = $db->getResult();
    if (!empty($result)) {
    ...
    if ($result == 1) {
        $error['add_bhagawath_geetha'] = "<section class='content-header'>
                                            <span class='label label-success'>bhagawath_geetha Added Successfully</span> </section>";
    } else {
