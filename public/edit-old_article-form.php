   ...
   if (!empty($title) && !empty($description) )
    {
		if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
			...
			$sql = "UPDATE old_articles SET `image`='" . $upload_image . "' WHERE `id`=" . $ID;
			$db->sql($sql);
		}

		$sql = "UPDATE old_articles SET title='$title',description='$description' WHERE id=$ID";
		$db->sql($sql);
...
$sql_query = "SELECT * FROM old_articles WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "old_articles.php";
	</script>
<?php } ?>
<section class="content-header">
	<h1>
		Edit Festivals Info<small><a href='old_articles.php'><i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Festivals Info</a></small></h1>
	...
	<div class="row">
		<div class="col-md-12">
			...
			<div class="box box-primary">
				...
				<!-- /.box-header -->
				<!-- form start -->
				<form id="edit_old_article_form" method="post" enctype="multipart/form-data">
					<div class="box-body">
