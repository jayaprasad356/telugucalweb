...
if (isset($_POST['btnEdit'])) {
	...
	$error = array();
	  ...
	  if ( !empty($month) && !empty($text1))
        {
             $sql_query = "UPDATE abdhikam SET month='$month',year='$year',text1='$text1' WHERE id =$ID";
			 $db->sql($sql_query);
			 $res = $db->getResult();
			...
			if ($update_result == 1)
		   {
				...
				if (
                  ...
                  $error['update_abdhikam'] = " <section class='content-header'><span class='label label-success'>Child Birth Muhurtham updated Successfully</span></section>";
			} else {
				$error['update_abdhikam'] = " <span class='label label-danger'>Failed to update</span>";
			}
		}
	} 
...
$sql_query = "SELECT * FROM abdhikam WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

$sql_query = "SELECT * FROM abdhikam_variant WHERE abdhikam_id =" . $ID;
$db->sql($sql_query);
$resslot = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "abdhikam.php";
	</script>
<?php } ?>
<section class="content-header">
	<h1>
		Edit Abdhikam<small><a href='abdhikam.php'><i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Abdhikams</a></small></h1>
	<small><?php echo isset($error['update_abdhikam']) ? $error['update_abdhikam'] : ''; ?></small>
	<ol class="breadcrumb">
	...
	<div class="row">
		<div class="col-md-12">
			...
			<div class="box box-primary">
				...
				<!-- /.box-header -->
				<!-- form start -->
				<form id="edit_abdhikam_form" method="post" enctype="multipart/form-data">
					<div class="box-body">
					      <div class="row">
							...
							foreach ($resslot as $row) {
								...
								<div id="packate_div">
									<div class="row">
									    <input type="hidden" class="form-control" name="abdhikam_variant_id[]" id="abdhikam_variant_id" value='<?= $row['id']; ?>' />
										<div class="col-md-3">
										...
										<?php if ($i == 0) { ?>
												<div class='col-md-1'>
													<label>Tab</label>
													<a id="add_packate_variation" title='Add variation of Abdhikam' style='cursor: pointer;color:white;'><button class="btn btn-warning">Add more</button></a>
												</div>
											<?php } else { ?>
												<div class="col-md-1">
													<label>Tab</label>
													<a class="remove_variation text-danger" data-id="data_delete" title="Remove variation of Abdhikam" style="cursor: pointer;color:white;"><button class="btn btn-danger">Remove</button></a>
												</div>
											<?php } ?>
									</div>
								</div>
    ...
    $(document).on('click', '.remove_variation', function() {
        if ($(this).data('id') == 'data_delete') {
            if (confirm('Are you sure? Want to delete this row')) {
                var id = $(this).closest('div.row').find("input[id='abdhikam_variant_id']").val();
                $.ajax({
