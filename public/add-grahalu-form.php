...
if (isset($_POST['btnAdd'])) {
       ...
       if (!empty($name)) {
            ...
            // upload new image
            $upload = move_uploaded_file($_FILES['grahalu_image']['tmp_name'], 'upload/grahalu/' . $menu_image);

            // insert new data to menu table
            $upload_image = 'upload/grahalu/' . $menu_image;

         
            $sql_query = "INSERT INTO grahalu (name,image)VALUES('$name','$upload_image')";
            $db->sql($sql_query);
            $result = $db->getResult();
            if (!empty($result)) {
                $result = 0;
            ...
            if ($result == 1) {
                ...
                $error['add_grahalu'] = "<section class='content-header'>
                                                <span class='label label-success'>Grahalu Added Successfully</span> </section>";
            } else {
                $error['add_grahalu'] = " <span class='label label-danger'>Failed</span>";
            }
            }
        }
?>
<section class="content-header">
    <h1>Add Grahalu <small><a href='grahalu.php'> <i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Grahalu</a></small></h1>

    <?php echo isset($error['add_grahalu']) ? $error['add_grahalu'] : ''; ?>
    <ol class="breadcrumb">
