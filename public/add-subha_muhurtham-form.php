...
if (isset($_POST['btnAdd'])) {
        ...
        {
                $sql_query = "INSERT INTO subha_muhurtham (month,text1,year)VALUES('$month','$text1','$year')";
                $db->sql($sql_query);
                $result = $db->getResult();
                if (!empty($result)) {
                    $result = 0;
                ...
                if ($result == 1) {
                    $sql = "SELECT id FROM subha_muhurtham ORDER BY id DESC LIMIT 1";
                    $db->sql($sql);
                    $res = $db->getResult();
                    $subha_muhurtham_id = $res[0]['id'];
                    for ($i = 0; $i < count($_POST['date_month']); $i++) {
                        ...
                        $sql = "INSERT INTO subha_muhurtham_variant (subha_muhurtham_id,date_month,description) VALUES('$subha_muhurtham_id','$date_month','$description')";
                        $db->sql($sql);
                        $subha_muhurtham_variant_result = $db->getResult();
                    }
                    if (!empty($subha_muhurtham_variant_result)) {
                        $subha_muhurtham_variant_result = 0;
                    } else {
                        $subha_muhurtham_variant_result = 1;
                    }
                    
                    $error['add_subha_muhurtham'] = "<section class='content-header'>
                                                    <span class='label label-success'>Subha Muhurtham Added Successfully</span> </section>";
                } else {
                    $error['add_subha_muhurtham'] = " <span class='label label-danger'>Failed</span>";
                }
                }
...
<section class="content-header">
    <h1>Add Subha Muhurtham<small><a href='subha_muhurtham.php'> <i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Subha Muhurthams</a></small></h1>

    <?php echo isset($error['add_subha_muhurtham']) ? $error['add_subha_muhurtham'] : ''; ?>
    <ol class="breadcrumb">
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="box box-primary">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form name="add_subha_muhurtham_form" method="post" enctype="multipart/form-data">
                    <div class="box-body">
                           <div class="row">
                            ...
                            <div id="packate_div"  >
                                <div class="row">
                                    ...
                                    <div class="col-md-1">
                                        <label>Tab</label>
                                        <a class="add_packate_variation" title="Add variation of Subha Muhurtham" style="cursor: pointer;color:white;"><button class="btn btn-warning">Add More</button></a>
                                    </div>
                                    <div id="variations">
                                    </div>
                                </div>
...
<script>
    $('#add_subha_muhurtham_form').validate({
